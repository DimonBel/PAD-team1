...
#### Update a Player Academic Record

`PATCH /api/players/{player_id}/academic-record`
Description: Moderator correction path — override a specific grade or diploma milestone (e.g.
fix a mis-graded exam) without forcing a retake or replaying the whole attempt flow.
Headers: `Authorization: Bearer <jwt>` (`moderator` role required)
Payload:
...
`DELETE /api/players/{player_id}/academic-record`
Description: Wipes a player's record. Exists for QA/test-lobby cleanup and GDPR-style
data-removal requests — the next graded attempt re-provisions a fresh record.
Headers: `Authorization: Bearer <jwt>` (`moderator` role required)
Success Response (200 OK):

```json
{ "player_id": "player-uuid-123", "deleted": true }
```
